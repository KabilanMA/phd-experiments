...
#include "backend_interface.hpp"       // FuzzBackend interface
...
int main(int argc, char* argv[]) {
    ...
    for (size_t iter = 0; iter < max_iterations && !g_terminate; ++iter) {
        try {
            ...
            // 3) Generate backend-specific kernel (atomic behavior *should* be implemented by plugin)
            // fs::path backend_kernel = iter_data_dir / "backend_kernel"; // plugin decides extension/format
